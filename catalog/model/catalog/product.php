...
class ModelCatalogProduct extends Model
{
	...
	public function getProducts($data = array())
	{
		...
		if($data['filter_name'] == 'saleoffs'){
			...
			$sql .= " LEFT JOIN shipping_method_lc smlc ON (p.product_id = smlc.product_id) LEFT JOIN ".DB_PREFIX."product_description pd ON (p.product_id = pd.product_id) LEFT JOIN ".DB_PREFIX."product_to_store p2s ON (p.product_id = p2s.product_id) INNER JOIN `shipping_method_lc` AS sm ON p.`product_id`=sm.`product_id` 
			...
			if(!empty($data['offers_delivery'])){
				$sql .= "AND smlc.vendor != ''";
			}
			
			if(!empty($data['warranty'])){
				// Todo: agregar cuando se mapee el dato
				$sql .= "AND smlc.vendor != ''";
			}
			
			if(!empty($data['is_saleoff'])){
				$sql .= "AND p.listing_type = 3";
			}
			
			
			if(!empty($data['condition'])){
				$sql .= " AND p.product_condition = '".$data['condition']."'";
			}
	...
	public function getProduct($product_id)
	{
						...
						SELECT 
							...
							(
								...
								p.sort_order FROM ".DB_PREFIX."product p 
									...
									INNER JOIN `shipping_method_lc` AS sm ON p.`product_id` = sm.`product_id`  
		...
		if($query->num_rows){
            ...
            {
                $query = $this->db->query("	SELECT listing_type	FROM oc_product	WHERE product_id = '$product_id'");
                if ($query->num_rows)
                {
                    return array(
    ...
    {
        $query = $this->db->query("	SELECT product_condition	FROM oc_product	WHERE product_id = '$product_id'");
        if ($query->num_rows)
        {
            return array(
	...
	public function getProductOptions($product_id)
	{
		...
		foreach($product_option_query->rows as $product_option){
			...
			$product_option_data[] = array('product_option_id' => $product_option['product_option_id'], 'product_option_value' => $product_option_value_data, 'option_id' => $product_option['option_id'], 'name' => $product_option['name'], 'type' => $product_option['type'], 'value' => $product_option['value'], 'required' => $product_option['required']);
		}
...
");
	...
	public function getVendor($product_id)
	{
		...
		if($q->num_rows){
			...
			if($q1->num_rows){
				return array('seller_id' => $seller_id, /*                     * vendor customer id* */
						'firstname' => $q1->row['firstname'], 'lastname' => $q1->row['lastname'], 'date_added' => $q1->row['date_added']);
			} else
				return false;
		} else
			return false;
