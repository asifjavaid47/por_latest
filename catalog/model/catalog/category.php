...
class ModelCatalogCategory extends Model {
	...
	{
		//if($this->db->query("insert into oc_question values('', '" . $data['customer_id'] . "', '" . $data['product_id'] . "', '" . $data['question'] . "', '" . $data['date_time'] . "' )"))
        if($this->db->query("insert into oc_question (customer_id, product_id, question, date_time) values('" . $data['customer_id'] . "', '" . $data['product_id'] . "', '" . $data['question'] . "', '" . $data['date_time'] . "' )"))
		{
			return 'inserted';
		}
		else
    ...
    public function test_question($addData) {
        $this->db->query("INSERT INTO oc_question (customer_id,product_id,question,report_id,q_role,date_time)VALUES('" . $this->db->escape($addData['customer_id']) . "','" . $this->db->escape($addData['product_id']) . "','" . $this->db->escape($addData['question']) . "','1','buyer','".$this->db->escape($addData['date_time'])."')");
    }
...
LIMIT 0, 3 ");
    ...
    public function get_seller_email($id) {
        $query = $this->db->query("SELECT ov.`customer_id`, oc.`customer_id`,oc.`email` FROM oc_vendordetail_lc ov INNER JOIN oc_customer oc ON  ov.`customer_id` = oc.`customer_id` WHERE ov.id = '" . $id . "' ");
        return $query->row['email'];
    }

    public function save_reporting($data) {
        $this->db->query("INSERT INTO  oc_reporting_lc (reporter_id,product_id,type_of_issue,comments,seller_id) VALUES ('" . $this->db->escape($data['reporter_id']) . "','" . $this->db->escape($data['product_id']) . "','" . $this->db->escape($data['type_of_issue']) . "','" . $this->db->escape($data['comments']) . "','" . $this->db->escape($data['seller_id']) . "');");
