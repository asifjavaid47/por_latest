...
class ControllerProductSearch extends Controller
{
	...
	public function index()
	{
		...
		if(isset($this->request->get['offers_delivery'])){
			$offers_delivery = $this->request->get['offers_delivery'];
		} else{
		...
		if(isset($this->request->get['is_saleoff'])){
			$is_saleoff = $this->request->get['is_saleoff'];
		} else{
			$is_saleoff = NULL;
		}
		...
		$data['breadcrumbs'][] = array('text' => $this->language->get('text_home'), 'href' => $this->url->link('product/search'));
		...
		if(isset($this->request->get['filters'])){
			$url .= '&filters='.urlencode(html_entity_decode($this->request->get['filters'], ENT_QUOTES, 'UTF-8'));
		}
		
		if(isset($this->request->get['condition'])){
			$url .= '&filters='.urlencode(html_entity_decode($this->request->get['condition'], ENT_QUOTES, 'UTF-8'));
		}
		...
		if(isset($this->request->get['is_saleoff'])){
			$url .= '&is_saleoff='.$this->request->get['is_saleoff'];
		}
		
		
			
		if(isset($this->request->get['offers_delivery'])){
			$url .= '&offers_delivery='.$this->request->get['offers_delivery'];
		}
		
		$data['breadcrumbs'][] = array('text' => $this->language->get('heading_title'), 'href' => $this->url->link('product/search', $url));
		if(isset($this->request->get['search'])){
			$data['heading_title'] = $this->language->get('heading_title').' - '.$this->request->get['search'];
		} else{
			$data['heading_title'] = $this->language->get('heading_title');
		...
		$data['products'] = array();
		if(isset($this->request->get['category_id']) || isset($this->request->get['search']) || isset($this->request->get['tag']) || isset($this->request->get['price'])){
			...
			foreach($results as $result){
				...
				if(isset($current_sort)){
					...
					$data['is_sort_set'] = 'yes';
					$data['sort_order'] = $order;
					$data['products'][] = array('current_unit_price' => $max_price, 'product_id' => $result['product_id'], 'thumb' => $image, 'name' => $result['name'], 'quantity' => $result['quantity'], 'location' => $result['location'], 'listing_type' => $result['listing_type'], 'require_shipping_method' => $result['require_shipping_method'], 'require_shipping_method' => $result['require_shipping_method'], 'buyer_pay_or_free_shipping' => $result['buyer_pay_or_free_shipping'], 'own_shipping_price' => $result['own_shipping_price'], 'radius' => $result['radius'], 'vendor_id' => $result['vendor_id'], 'product_condition' => $result['product_condition'], 'description' => utf8_substr(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8')), 0, $this->config->get($this->config->get('config_theme').'_product_description_length')).'..', 'price' => $price, 'special' => $special, 'tax' => $tax, 'minimum' => $result['minimum'] > 0? $result['minimum']:1, 'rating' => $result['rating'], 'href' => $this->url->link('product/product', 'product_id='.$result['product_id'].$url), 'date_added' => $result['date_added']);
				} else{
					$max_price = $this->model_catalog_product->getdicountedunitprice($result['product_id'], $quantity);
					$data['is_sort_set'] = 'no';
					$data['products'][] = array('current_unit_price' => $max_price, 'product_id' => $result['product_id'], 'thumb' => $image, 'name' => $result['name'], 'quantity' => $result['quantity'], 'location' => $result['location'], 'listing_type' => $result['listing_type'], 'require_shipping_method' => $result['require_shipping_method'], 'require_shipping_method' => $result['require_shipping_method'], 'buyer_pay_or_free_shipping' => $result['buyer_pay_or_free_shipping'], 'own_shipping_price' => $result['own_shipping_price'], 'radius' => $result['radius'], 'vendor_id' => $result['vendor_id'], 'product_condition' => $result['product_condition'], 'description' => utf8_substr(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8')), 0, $this->config->get($this->config->get('config_theme').'_product_description_length')).'..', 'price' => $price, 'special' => $special, 'tax' => $tax, 'minimum' => $result['minimum'] > 0? $result['minimum']:1, 'rating' => $result['rating'], 'href' => $this->url->link('product/product', 'product_id='.$result['product_id'].$url), 'date_added' => $result['date_added']);
			...
			$data['sorts'] = array();
			$data['sorts'][] = array('text' => $this->language->get('Más relevante'), 'value' => 'p.listing_type-DESC', 'href' => $this->url->link('product/search', 'sort=p.sort_order&order=ASC'.$url));
			$data['sorts'][] = array('text' => $this->language->get('text_price_asc'), 'value' => 'p.price-ASC', 'href' => $this->url->link('product/search', 'sort=p.price&order=ASC'.$url));
			$data['sorts'][] = array('text' => $this->language->get('text_price_desc'), 'value' => 'p.price-DESC', 'href' => $this->url->link('product/search', 'sort=p.price&order=DESC'.$url));
			...
			sort($limits);
			foreach($limits as $value){
				$data['limits'][] = array('text' => $value, 'value' => $value, 'href' => $this->url->link('product/search', $url.'&limit='.$value));
			}
			$url = '';
			if(isset($this->request->get['search'])){
				$url .= '&search='.urlencode(html_entity_decode($this->request->get['search'], ENT_QUOTES, 'UTF-8'));
		...
		$sidebarLocations = $this->model_catalog_category->getLocations();
		foreach($sidebarLocations as $location){
			$data['locations'][] = array('location' => $location['location'], 'locProducts' => $location['locProducts']);
		}
		if($this->request->server['HTTPS']){
			$server = $this->config->get('config_ssl');
		} else{
