...
// if (!pspell_check($pspell_link, "telefono"))
// {
	// echo 'in condition';
    // $suggestions = pspell_suggest($pspell_link, "telefono");
	// echo '<pre>';
	// print_r($suggestions);
// }
// exit;
...
?>
	...
	<script>
			...
			$.ajax(
				{
					...
					success: function(data){
						...
						if(parseInt(<?php echo $logged_in_seller?>) !== parseInt(parsed_data['product_details'][0].seller_id))
						{
							if(parseInt(parsed_data['reported_by_user']))
							{
								$('#reporting_product_div').html('<span>You reported this product!</span>');
							}
							else
							{
								$('#reporting_product_div').html('<a class="buy-btn btncustom pull-right" id="report_product" data-toggle="modal" data-target="#myModal"  href="#"> Report</a>');
							}
						}
						...
						{
							$('#discounted_prices_list').append('<div class="first-table-row"><a class="for_first_price prevent_default change_selected_price hover_selected" selector_attribute = "0" href="#"><div class="table-logo"><span class="hover-hide">x</span><img src="<?php echo $base; ?>image/assets/images/hover-logo-img.png" alt="" class="hover-show"></div><div class="unit-count-block">1</div><div class="price-table-block">$'+parseFloat(parsed_data['product_details'][0].price).toFixed(2)+'</div></a><div class="clr"></div></div>');
							product_data_matched = 1;
						}
						else
						{
							$('#discounted_prices_list').append('<div class="first-table-row"><a class="for_first_price prevent_default change_selected_price" selector_attribute = "0" href="#"><div class="table-logo"><span class="hover-hide">x</span><img src="<?php echo $base; ?>image/assets/images/hover-logo-img.png" alt="" class="hover-show"></div><div class="unit-count-block">1</div><div class="price-table-block">$'+parseFloat(parsed_data['product_details'][0].price).toFixed(2)+'</div></a><div class="clr"></div></div>');
						}
						...
						{
							product_all_prices[i+1] = parseFloat(parsed_data['product_discounts'][i].price).toFixed(2);
						}
						...
						for(i=0; i < number_of_discounted_prices; i++)
						{
							...
							{
								if(Math.round(parsed_data['product_discounts'][i].quantity) === product_quantity_final && parseFloat(parsed_data['product_discounts'][i].price).toFixed(2) === product_amount_final)
								{
									$('#discounted_prices_list').append('<div class="first-table-row"><a id="'+product_all_prices[i+1]+'" class="hover_selected prevent_default change_selected_price first_price" selector_attribute = "'+(i+1)+'"  href="#" ><div class="table-logo"><span class="hover-hide">x</span><img src="<?php echo $base; ?>image/assets/images/hover-logo-img.png" alt="" class="hover-show"></div><div class="unit-count-block">'+Math.round(parsed_data['product_discounts'][i].quantity)+'</div><div class="price-table-block">$'+parseFloat(parsed_data['product_discounts'][i].price).toFixed(2)+'</div></a><div class="clr"></div></div>');
									product_data_matched = 1;
								}
								else
								{
		...
		//send report on clicking button
...
<<<<<<< .mine
			<div id='reporting_product_div' style='float:right'>
			</div>
=======
			<label class="prdtype" style="visibility:hidden" id="type<?php echo $listing_type ?>"> tipo de liquidacion:<?php echo $listing_type ?></label>
			<script type="text/javascript">
				$(document).ready(function()
				{
					if( $(".prdtype").is("#type3") === true )
					{
						$('.producto-liquidacion').css('display', 'inline');
					}
				});
			</script>

			<a class="buy-btn btncustom pull-right" id="report_product" data-toggle="modal" data-target="#myModal"  href="#"> Report</a>
>>>>>>> .r423
		...
		<div class="row">
			...
			<!-- foto principal -->
			<span style="display:none;width:120px; color:red; padding-left:18px;" class="producto-liquidacion">liquidacion</span>
			<label style="color:red; padding-left:18px;" ><?php echo $product_condition ?></label>
			...
			<div class="col-sm-6">
				...
				<div class="col-sm-6" style="padding-bottom:30px">
					...
					<div class="social-block-product">
						<a href="javascript:void(0)" class="add_to_wishlist" title="Add to Wishlist" product_id="<?php if(isset($_GET['pxs'])) echo $_GET['pxs']; ?>" onclick="" ><i class="fa fa-heart"></i></a>
					</div>
	...
	<!--Modal to show for the reporting options-->
